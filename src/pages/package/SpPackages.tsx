...
const SpPackages = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any) => (
        <span
          className={`p-1 text-white rounded-lg ${text ? "bg-green-300" : "bg-red-300"}`}
        >
        ...
        <Dropdown
          overlay={
            <Menu>
              <Menu.Item
                onClick={() =>
                  navigate("/packages/viewPackage", { state: { record } })
                }
              >
                View
              </Menu.Item>
  ...
  return (
    <div>
      ...
      <Card
        title={<h3 className="font-bold text-sm text-left">Packages List</h3>}
        className="mt-5"
      >
        <div className="table-responsive">
          ...
          <Table
            id={tableId}
            columns={columns}
            dataSource={systemPackages}
            loading={loadingSystemPackages}
          />
        </div>
      </Card>
